...
import { mapperStatus, mapperKeterangan, dateParsing, dueDateParsing } from "@/utils/helper";
</script>
              ...
              <div class="w-[90px] ml-2 filter-container">
                ...
                <div v-show="isFilterStatus"
                  ...
                  <ul class="text-sm w-full">
                    ...
                    <li @click="updateFilterStatus('Ditolak')"
                      class="border-b h-[40px] flex items-center group hover:bg-[#E9F1FB]">
                      <input type="checkbox" class="w-4 h-4 border-[2px] ml-4 accent-[#2671D9]"
                        :checked="filterStatus === 'Ditolak'" readonly>
                      <span class="text-[#333333] ml-2 group-hover:font-semibold group-hover:text-[#2671D9]"
                        :class="{ 'text-[#2671D9] font-semibold': filterStatus === 'Ditolak' }">Ditolak</span>
                    </li>
                  </ul>
                </div>
              ...
              <table class="table-auto w-full text-left">
                ...
                <tbody>
                  <tr v-for="row in paginatedRows" :key="row.id"
                    class="bg-white border-b text-sm text-[#333333] h-[77px]">
  ...
  methods: {
    ...
    async getDataApi() {
      ...
      if (positionLevel == "manager") {
        url = "manager/mounda/finish";
      }
      if (positionLevel == "vp") {
        url = "vp/mounda/finish";
      }
      if (positionLevel == "direksi") {
        url = "direksi/mounda/finish";
      }
      if (!url) {
      ...
      let url2 = null;
      if (positionLevel == "manager") {
        url2 = "manager/pks/finish";
      }
      if (positionLevel == "vp") {
        url2 = "vp/pks/finish";
      }
      if (positionLevel == "direksi") {
        url2 = "direksi/pks/finish";
      }
      if (!url2) {
        this.isLoading = false;
        return modalFailed.value = {
