									...
									<tr v-for="(row, index) in paginatedRows" :key="row.id"
										...
										<td class="w-[54px] px-3">
											...
											<div v-if="activeViewIndex === index" class="absolute -translate-x-[135px] w-[160px]">
												<!-- PKS type -->
												<router-link v-if="row.type === 'PKS'" :to="{ path: `/Detaildraft/${row.type}/${row.code}` }">
													<div
												...
												<!-- MoU type -->
												<router-link v-if="row.type === 'MoU'" :to="{ path: `/Detaildraft/${row.type}/${row.code}` }">
													<div
														class="h-[40px] rounded-lg border-[1px] border-[#E5E7E9] flex cursor-pointer shadow-lg bg-white hover:bg-gray-200 hover:border-[#2671D9]">
														<svg width="16" height="12" class="mx-3 my-3" viewBox="0 0 16 12" fill="none"
															xmlns="http://www.w3.org/2000/svg">
												...
												<!-- NDA type -->
												<router-link v-if="row.type === 'NDA'" :to="{ path: `/Detaildraft/${row.type}/${row.code}` }">
													<div
														class="h-[40px] rounded-lg border-[1px] border-[#E5E7E9] flex cursor-pointer shadow-lg bg-white hover:bg-gray-200 hover:border-[#2671D9]">
														<svg width="16" height="12" class="mx-3 my-3" viewBox="0 0 16 12" fill="none"
															xmlns="http://www.w3.org/2000/svg">
...
export default {
	...
	methods: {
		...
		async getDataApi() {
			...
			const res = await fetchGet("staff/mounda/draft", null, this.$router);
			if (res.status == 200) {
			...
			const res2 = await fetchGet("staff/pks/draft", null, this.$router);
			if (res2.status == 200) {
